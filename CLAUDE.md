...
- **`--group-add render/video` on Ubuntu-based images**: Ubuntu Docker images don't ship `render` or `video` groups. Podman looks up `--group-add <name>` in the container's `/etc/group` at start; if the group is missing, the container fails to start. Fix by adding `RUN groupadd -f render && groupadd -f video` to the Dockerfile. Fedora images have both groups already.
