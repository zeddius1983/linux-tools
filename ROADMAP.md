...
- [x] `nvidia-cdi-regenerate` — host-only boot service; regenerates the CDI spec each boot to match installed GPUs
- [x] `nvidia-p2p-acs` — host-only boot service; clears PCIe ACS redirect for switch-local GPU P2P (renumber-proof)
