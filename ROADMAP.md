...
- [ ] **ROCm 10.0 migration** — ROCm 10.0 (27 Aug 2026) is the first major version since 7.x, built end to end on TheRock with a ~6-week cadence, and lists gfx1151 (Ryzen AI Max+ 395 / Radeon 8060S) as officially supported. It names Unsloth support on Ryzen AI MAX and ComfyUI tuning for Radeon/Ryzen as release features. What was pullable on 13 Sep 2026, per app:
  - [x] `llama-cpp` — `rocm/dev-ubuntu-24.04:10.0.0-full` is now the **ROCm 10.0** choice in the setup wizard, next to ROCm 7.2 (still the default) and CUDA. The feared `-DGGML_HIP_ROCWMMA_FATTN=ON` breakage was moot — upstream had already removed the option. A first short `llama-bench` (Qwen3.5-9B IQ2_XXS) put ROCm 10 at 1075 vs 955 tok/s pp512 on ROCm0, with tg64 about even (40.5 vs 39.9). Needed before flipping the default: a longer benchmark on real model sizes, and some mileage on ROCm 10 boxes. The image registers no ROCm libs with ld.so — any other app moving to it will hit the same silent "no ROCm device" failure (see `apps/llama-cpp/.memory.md`).
