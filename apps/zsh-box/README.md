...
```bash
distrobox enter zsh-box-box -- zsh-install --tools "fzf bat eza zoxide starship"
```
...
| Path | Owner | Behavior |
|---|---|---|
| `~/.config/zsh/zsh-box.zsh` | **zsh-box** | Fully regenerated on every run. Carries a `DO NOT EDIT` header. Holds core options, history, completion, keybindings, plugin sourcing, the `conf.d` loader, and the prompt. |
| `~/.zshrc` | **you** | Written **once** as a thin loader that `source`s `zsh-box.zsh`, then never overwritten. Add your own customizations below the source line — they survive re-runs. |
| `~/.config/zsh/conf.d/NN-tool.zsh` | **zsh-box** | Per-tool init snippets (aliases, env, `eval "$(tool init zsh)"`), sourced in order by the glob loop in `zsh-box.zsh`. Removed automatically when a tool is deselected. |
...
| `~/.config/zsh/` | `zsh-box.zsh` + `conf.d/` fragments |
...
- Re-run `tools setup zsh-box` (or `zsh-install --tools "…"`) any time to add/remove tools or pull updated binaries.
